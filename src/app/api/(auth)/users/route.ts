...
export const PATCH = async (request: Request) => {
    try {
        ...
        return new NextResponse(
            JSON.stringify({ message: "User is updated" }), { status: 200 }
        );
...
export const DELETE = async (request: Request) => {
    try {
        ...
        return new NextResponse(
            JSON.stringify({ message: "User is deleted" }), { status: 200 }
        )
        await connect();
    } catch (error: any) {
        return new NextResponse(
            JSON.stringify({ message: "Error in deleting user" + error.message }), { status: 500 }
        )
    }
}